...
SWEP.Secondary.Ammo			= "Aeolis_Flame" -- Self explanitory, ammo type.
SWEP.NextGen = 0
SWEP.Secondary.MaxAmmo = 50

DEFINE_BASECLASS( SWEP.Base )

CreateClientConVar( "cso_flamethrower_fx", 2, false, false )

function SWEP:SecondaryAttack(...)
	if (SERVER) and not self:GetSprinting() and TFA.Enum.ReadyStatus[ self:GetStatus() ] and self:Ammo2() > 0 then
	...
	util.Effect("cso_flamethrower",flamefx,true,true)
	self:ShootBulletCustom()
	self:TakeSecondaryAmmo(1)
	self:SetNextSecondaryFire(CurTime()+0.05)
	self:SetNextPrimaryFire(CurTime()+0.1)
	self:SendViewModelAnim( ACT_VM_SECONDARYATTACK)
	if self.NextIdleSound and CurTime() > self.NextIdleSound and self:Ammo2() > 0 then
	self:EmitSound("Flamethrower.Fire")
	self.NextIdleSound = CurTime() + SoundDuration( "Flamethrower.Fire" ) - 0.1
	end
end
...
function SWEP:Think2( ... )
	...
	local shooting = self:GetOwner():KeyDown(IN_ATTACK2) and TFA.Enum.ReadyStatus[ self:GetStatus() ]
	if shooting ~= self.Shooting_Old and TFA.Enum.ReadyStatus[ self:GetStatus() ] and self:Ammo2() > 0 then
		if shooting and self:GetOwner():KeyDown(IN_ATTACK2) then
			self.NextIdleSound = CurTime() + 0
		else
			self:EmitSound("Flamethrower.Fire_End")
...
function SWEP:ShootBulletCustom()
...
if SERVER then
if self.NextGen >= 2 and self:Ammo2() < 50 then
