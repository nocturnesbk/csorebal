...
SWEP.PrintName				= "Air Burster"		-- Weapon name (Shown on HUD)
...
SWEP.Primary.DamageType = DMG_FALL --See DMG enum.  This might be DMG_SHOCK, DMG_BURN, DMG_BULLET, etc.  Leave nil to autodetect.  DMG_AIRBOAT opens doors.
SWEP.Primary.Force = nil --Force value, leave nil to autocalc
SWEP.Primary.HullSize = 30 --Big bullets, increase this value.  They increase the hull size of the hitscan bullet.
...
SWEP.NZPaPName				= "Flight Simulator"
...
SWEP.Primary.Range = 400 --Flame Distance
...
--[[VIEWMODEL]]--
SWEP.ViewModel			= "models/weapons/tfa_cso/c_airburster.mdl" --Viewmodel path
...
--}
--[[WORLDMODEL]]--
SWEP.WorldModel			= "models/weapons/tfa_cso/w_airburster.mdl" -- Weapon world model path
SWEP.Bodygroups_W = nil --{
--[0] = 1,
--[1] = 4,
--[2] = etc.
--}
SWEP.DefaultHoldType = "ar2" -- This is how others view you carrying the weapon. Options include:
...
CreateClientConVar( "cso_airburster_fx", 2, false, false )
...
function SWEP:PrimaryAttack(...)
	...
	util.Effect("cso_airburster",flamefx,true,true)
end
...
function SWEP:Think2( ... )
	...
	if shooting ~= self.Shooting_Old then
		if shooting then
			self:EmitSound("AirBurster.Fire")
			self.NextIdleSound = CurTime() + 0
		else
			self:EmitSound("AirBurster.Fire_End")
	...
	if shooting then
		if self.NextIdleSound and CurTime() > self.NextIdleSound then
			self:EmitSound("AirBurster.Fire")
			self.NextIdleSound = CurTime() + SoundDuration( "AirBurster.Fire" ) - 0.1
		end
	end
...
function SWEP:ShootBullet()
	...
	for k, target in pairs( ents.FindInCone( self.Owner:GetPos(), self.Owner:GetAimVector(), 225, 0.975 ) ) do
...
function SWEP:SecondaryAttack( ... )
	if CurTime() >  self:GetNextPrimaryFire() and TFA.Enum.ReadyStatus[ self:GetStatus() ] and not self:GetSprinting() and self:Clip1() > 0 then
		self:EmitSound("AirBurster.Fire2")
		...
		if SERVER then
			...
			util.Effect("exp_airburster", effectdata)
			local dmginfo = CSO:DMGInfo(self.Owner, self, 85, Vector(0,0,0), DMG_SONIC, self:GetPos())
			CSO:DoRadiusAttack(self, dmginfo, 225, true, 4196, 2, false, nil, true)
		end
	end
...
function SWEP:Holster( ... )
	self:StopSound("AirBurster.Idle")
	return BaseClass.Holster(self,...)
end

if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_airburster")
