...
SWEP.Author				= "★BulletStorm★" --Author Tooltip
...
SWEP.Primary.SilencedSound = Sound("BALROG1.Fire2") -- This is the sound of the weapon, when silenced.
SWEP.Primary.Damage_Regular		= 55					-- Damage, in standard damage points.
SWEP.Primary.Damage		= SWEP.Primary.Damage_Regular
SWEP.Primary.Damage_Secondary		= 165					-- Damage, in standard damage points.
...
SWEP.Primary.SpreadMultiplierMax = 4.25 --How far the spread can expand when you shoot.
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_balrog1.mdl" --Viewmodel path
...
SWEP.IronSightsPos = Vector(7.239, 0, -0.115)
...
SWEP.BlowbackEnabled = fasle --Enable Blowback?
...
function SWEP:Think2(...)
	if self:GetStatus() == TFA.Enum.STATUS_SILENCER_TOGGLE and self:GetStatusEnd() <= CurTime() then
		...
		if self:GetSilenced() then
			...
			self.Primary_TFA.DamageType = bit.bor(DMG_BLAST,DMG_ALWAYSGIB,DMG_BURN)
			self.Primary_TFA.BlastRadius	= 100
			self.Primary_TFA.RPM = 30
			self.Primary_TFA.AmmoConsumption = 1
			self.Primary_TFA.NumShots = 1
			self.MuzzleFlashEffectSilenced = "tfa_muzzleflash_generic"
			self.Primary_TFA.ImpactEffect = "exp_balrog1"
		    self:ClearStatCache()
		end
	end
