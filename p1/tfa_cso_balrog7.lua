-- i think the whole balrog set has to go. we'll see if XI is worth keeping
...
SWEP.Primary.Damage_Secondary		= 150					-- Damage, in standard damage points.
...
function SWEP:PostPrimaryAttack() -- global here. WHAT HAPPENED
	if self:Clip1() == 111 then
		...
		self.Primary_TFA.BlastRadius	= 128
		self.MuzzleFlashEffect = "cso_muz_balrog"
	end
	...
	if self:Clip1() == 0 or self:Clip1() == 119 then
		self.Primary_TFA.Damage = self.Primary.Damage_Regular
		self.Primary_TFA.DamageType = DMG_BULLET
		self.MuzzleFlashEffect = "tfa_muzzleflash_generic"
	end
...
SWEP.Primary.ImpactEffect = "exp_balrog7"
