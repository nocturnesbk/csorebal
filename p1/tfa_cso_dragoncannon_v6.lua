...
SWEP.PrintName				= "Black Dragon Cannon Expert"		-- Weapon name (Shown on HUD)
...
SWEP.ProjectileEntity = "black_dragon_cannon_blast" --Entity to shoot
SWEP.ProjectileVelocity = 500 --Entity to shoot's velocity

--[[VIEWMODEL]]--

SWEP.ViewModel			= "models/weapons/tfa_cso/c_black_dragon_cannon_v6.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_black_dragon_cannon_v6.mdl" -- Weapon world model path
...
SWEP.Base				= "tfa_gun_base"
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_dragoncannon_v6")
...
function SWEP:SecondaryAttack( ... )
		if self:Ammo1() > 0 and not self:GetSprinting() and TFA.Enum.ReadyStatus[ self:GetStatus() ] then
			self.Secondary.Automatic = true
			self.Primary_TFA.Projectile = "black_dragon_cannon_nade"
			self.Primary_TFA.ProjectileVelocity = 1000
			self.Primary_TFA.Sound = Sound("Cannon.Fire2")
			BaseClass.PrimaryAttack( self, ... )
		elseif self:GetOwner():KeyDown(IN_ATTACK2) then
			self.Primary_TFA.Projectile = "black_dragon_cannon_blast"
			self.Primary_TFA.ProjectileVelocity = 500
			self.Primary_TFA.Sound = Sound("Cannon.Fire")
