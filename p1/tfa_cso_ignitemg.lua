...
SWEP.PrintName				= "IGNITE-7"		-- Weapon name (Shown on HUD)
...
--Firing related
SWEP.Primary.Sound 			= Sound("IgniteMG.Fire")				-- This is the sound of the weapon, when you shoot.
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_ignitemg.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_ignitemg.mdl" -- Worldmodel path
...
SWEP.ShellTime	= 1.2 -- For shotguns, how long it takes to insert a shell.
SWEP.LoopedReload = false -- Enable looped / shotgun style / one round at time reloading.
SWEP.LoopedReloadInsertAmount = 30 -- How much rounds to insert on each reload cycle
...
SWEP.Base				= "tfa_gun_base"
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_ignitemg")
...
SWEP.LaserAnimation = {
	...
	["loop"] = {
		["type"] = TFA.Enum.ANIMATION_SEQ, --Sequence or act
		["value"] = "shoot2_loop", --Number for act, String/Number for sequence
		["is_idle"] = true
	},--looping animation
	["end"] = {
		["type"] = TFA.Enum.ANIMATION_SEQ, --Sequence or act
...
SWEP.LaserSound_Start = "IgniteMG.Shoot_Start"
SWEP.LaserSound_Loop = "IgniteMG.Shoot_Loop"
SWEP.LaserSound_End = "IgniteMG.Shoot_End"
...
SWEP.LaserDrain_Loop = 50 / 10--Ammo per second

SWEP.Secondary.Damage = 126
SWEP.Secondary.Reach = 1150
...
SWEP.Secondary.Ammo			= "Ignelium" -- Self explanitory, ammo type.
...
function SWEP:LaserThink()
	...
	if self:Ammo2() > 0 and self.Owner:KeyDown(IN_ATTACK2) and TFA.Enum.ReadyStatus[stat] and not self:GetSprinting() then
		...
		if self:CanSoundPredict() then
			...
			self.LaserSound_Start = SysTime() + 0.1
		end
	elseif stat == TFA.GetStatus("laser_attack") and ( ( not self.Owner:KeyDown(IN_ATTACK2) ) or self:Ammo2() <= 0 or self:GetSprinting() ) then
		...
		self:StopSound("IgniteMG.Shoot_Loop")
		if self:CanSoundPredict() then
		...
		if game.GetTimeScale() > 0.99 then
			self.Owner:FireBullets({
				...
				Callback = function(a, b, c)
					...
					b.Entity:Ignite( c:GetDamage(), 126 )
				end
			end
			})
		else
...
function SWEP:Think()
   if SERVER then
      if self.Secondary.Nextregen <= CurTime()  and self:Ammo2() < 25 then
         self.Owner:GiveAmmo( 1,self.Secondary.Ammo, true )                     -------SECONDARY AMMO REGEN
