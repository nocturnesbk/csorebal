...
SWEP.PrintName				= "Laser Storm"		-- Weapon name (Shown on HUD)	
...
--Firing related
SWEP.Primary.Sound 			= Sound("LaserStorm.Fire")				-- This is the sound of the weapon, when you shoot.
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_laser_storm.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_laser_storm.mdl" -- Worldmodel path
...
SWEP.TracerName 		= "cso_tra_laser_storm" 	--Change to a string of your tracer name.  Can be custom.
...
SWEP.Base				= "tfa_gun_base"
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_laser_storm")
...
SWEP.LaserAnimation = {
	["start"] = {
		...
		["value"] = "shoot_b", --Number for act, String/Number for sequence
		["is_idle"] = true
	},--Start animation
	["loop"] = {
		["type"] = TFA.Enum.ANIMATION_SEQ, --Sequence or act
	...
	["end"] = {
		["type"] = TFA.Enum.ANIMATION_SEQ, --Sequence or act
		["value"] = "shoot_b_end", --Number for act, String/Number for sequence
		["transition"] = true
	} --Outward transition
}

SWEP.LaserSound_Loop = "LaserStorm.Fire2_Loop"
SWEP.LaserSound_End = "LaserStorm.Fire2_End"
...
SWEP.LaserDrain_Loop = 160 / 10--Ammo per second

SWEP.Secondary.Damage = 90
SWEP.Secondary.Reach = 999999*999999
SWEP.Secondary.RPM = 1000
SWEP.Secondary.ClipSize			= -1					-- Size of a clip
SWEP.Secondary.DefaultClip			= 0					-- Default ammo to give...
SWEP.Secondary.MaxAmmo = 90
SWEP.Secondary.NumShots = 1
SWEP.Secondary.Automatic			= true					-- Automatic/Semi Auto
SWEP.Secondary.Ammo			= "Laserstorm_Alt" -- Self explanitory, ammo type.
	...
	if lasertrace.Hit then
		if game.GetTimeScale() > 0.99 then
			self.Owner:FireBullets({
				...
				HullSize = 15,
				Tracer = 1,
				TracerName = "cso_tra_laserstorm",
				Src = self.Owner:GetShootPos(),
				Dir = lasertrace.Normal,
				Callback = function(a, b, c)
					c:SetDamageType( bit.bor(DMG_AIRBOAT,DMG_DISSOLVE,DMG_BLAST) )
				end
			})
		else
			...
			dmg:SetDamageForce(self.Owner:GetAimVector() * (90))
			dmg:SetDamage(90)
			dmg:SetDamageType( bit.bor(DMG_AIRBOAT,DMG_DISSOLVE,DMG_BLAST) )
...
if SERVER then
if self.NextGen >= 1 and self:Ammo2() < 90 then
