...
SWEP.PrintName				= "SKULL-3"		-- Weapon name (Shown on HUD)
...
SWEP.Primary.Damage_Regular		= 35					-- Damage, in standard damage points.
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_skull3.mdl" --Viewmodel path
...
SWEP.Attachments = {
    [1] = { atts = { "cso_skull3blood"} },
}

--[[WORLDMODEL]]--

SWEP.WorldModel			= "models/weapons/tfa_cso/w_skull3.mdl" -- Worldmodel path
...
SWEP.IronSightsPos = Vector(7.2, -3.106, 0.879)
...
function SWEP:Think2(...)
	if self:GetStatus() == TFA.Enum.STATUS_SILENCER_TOGGLE and self:GetStatusEnd() <= CurTime() then
		...
		if self:GetSilenced() then
			...
			self.TracerName = nil
			self.Primary_TFA.RPM = 1200
			self.Primary_TFA.AmmoConsumption = 1
			self.Primary_TFA.NumShots = 1
			self.MuzzleFlashEffect = "tfa_muzzleflash_smg"
			self.Primary_TFA.ClipSize = 70
			self.Primary_TFA.KickUp	= 0.20	
            self.Primary_TFA.KickDown = 0.10				
            self.Primary_TFA.KickHorizontal	= 0.18
			self.Primary_TFA.Spread		= .0112
            self.Primary_TFA.IronAccuracy = .0032
			self.data.ironsights = 0
			self.MoveSpeed = 0.85
			self.WorldModel			= "models/weapons/tfa_cso/w_skull3_b.mdl"
			self.HoldType = "duel"
			self.Offset = {
			Pos = {
			Up = -5,
			...
			self.WElements = {
				["other_skull3"] = { type = "Model", model = "models/weapons/tfa_cso/w_skull3.mdl", bone = "ValveBiped.Bip01_L_Hand", rel = "", pos = Vector(5.75, 1, 5.5), angle = Angle(0, 80, 0), size = Vector(1.2, 1.2, 1.2), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
			}
			self.Akimbo = true
			...
			self.Primary_TFA.AmmoConsumption = 1
			self.Primary_TFA.NumShots = 1
			self.MuzzleFlashEffectSilenced = "tfa_muzzleflash_smg"
			self.Primary_TFA.ClipSize = 35
			self.Primary_TFA.KickUp	= 0.15	
            self.Primary_TFA.KickDown	= 0.05		
            self.Primary_TFA.KickHorizontal	= 0.12	
			self.Primary_TFA.Spread = .0064
			self.Primary_TFA.IronAccuracy = .0032
			self.data.ironsights = 1
			self.MoveSpeed = 1
			self.WorldModel			= "models/weapons/tfa_cso/w_skull3.mdl"
			self.HoldType = "smg"
			self.Offset = {
			Pos = {
			Up = -5.5,
			...
			self.Akimbo = false
			self.RunSightsPos = Vector(-5.788, -1.009, 0)
			self.RunSightsAng = Vector(-16.223, -35.562, 0)
			self.WElements = nil
			self.InspectPos = nil
			self.InspectAng = nil
			self:Unload()
