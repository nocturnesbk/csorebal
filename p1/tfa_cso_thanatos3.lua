...
SWEP.Purpose				= "Firing 12 rounds gives you two blades. Once you've got 6 blades, activate secondary mode to fire 6 explosive blades at once." --Purpose Tooltip
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_thanatos3.mdl" --Viewmodel path
...
SWEP.Secondary.MaxAmmo = 6
SWEP.Secondary.Ammo = "Thanatos3_Blade"
SWEP.Secondary.Automatic = true
SWEP.Secondary.Model = "models/weapons/tfa_cso/thanatos3_knife.mdl" --Projectile Model
...
function SWEP:SecondaryAttack()
	...
	if self:Ammo2() < 6 then return end
	if self:GetSilenced() and CurTime() > self:GetNextPrimaryFire() and TFA.Enum.ReadyStatus[ self:GetStatus() ] and not self:GetSprinting() and self:Ammo2() > 0 then
		self:TakeSecondaryAmmo(6)
		self:GetOwner():SetAnimation(PLAYER_ATTACK1)
		self.Secondary.Nextregen = CurTime() + 2
		local vel = self.Owner:GetForward() * 6000 - self.Owner:GetRight()  * 500
		for i=6,1,-1 do  --i = 6 - 6 shoots
		if SERVER then
...
if SERVER then
if self.NextGen >= 12 and not self:GetSilenced() and self:Ammo2() < 6 then
...
function SWEP:Holster( ... )
	self:StopSound("THANATOS3.Idle_W3")
