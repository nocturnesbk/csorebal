...
SWEP.Purpose				= "This gun's secondary mode fires spinning blades. These blades deal damage over time." --Purpose Tooltip
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_thanatos7.mdl" --Viewmodel path
...
SWEP.Secondary.Ammo = "Thanatos7_Blade"
SWEP.Secondary.Ent = "thanatos7_blade"
SWEP.Secondary.Automatic = true
SWEP.Secondary.Model = "models/weapons/tfa_cso/thanatos7_scythe.mdl" --Projectile Model
SWEP.Secondary.Damage = 50 --Grenade Damage
SWEP.Secondary.Velocity = 2500 --Grenade Velocity
SWEP.Secondary.Delay = 4 --Delay in seconds
SWEP.Secondary.Sound = Sound("THANATOS7.ScytheShoot") --Firing sound
...
function SWEP:SecondaryAttack()
	...
	if self:GetSilenced() and CurTime() > self:GetNextPrimaryFire() and TFA.Enum.ReadyStatus[ self:GetStatus() ] and not self:GetSprinting() and self:Ammo2() > 0 then
		...
		self.Secondary.Nextregen = CurTime() + 4 
		if SERVER then
...
self.Owner:GiveAmmo( 1,"Thanatos7_Blade", true )                     -------SECONDARY AMMO REGEN
self.Secondary.Nextregen =  CurTime()  + 3   ---2 seconds
end
end
