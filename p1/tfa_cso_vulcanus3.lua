...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_vulcanus3.mdl" --Viewmodel path
...
["ValveBiped.Bip01_L_Forearm"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, -14), angle = Angle(0, 0, 0) }
}
...
SWEP.MuzzleFlashEffectSilenced = "" --Change to a string of your muzzle flash effect.  Copy/paste one of the existing from the base.
...
function SWEP:SecondaryAttack()
	if self:Clip1() <= 3 and not self:GetSilenced() then return end
...
function SWEP:Think2(...)
	if self:GetStatus() == TFA.Enum.STATUS_SILENCER_TOGGLE and self:GetStatusEnd() <= CurTime() then
		...
		if self:GetSilenced() then
			self.TracerName = nil
			self.Primary_TFA.Damage = 39
			self.Primary_TFA.AmmoConsumption = 1
			self.Primary_TFA.NumShots = 1
	        self.Primary_TFA.Spread	= .01
			self:StopSound("Vulcanus3.IdleA")
			self:ClearStatCache()
		else
			self.TracerName = nil
			self.Primary_TFA.Damage = 48
			self.Primary_TFA.AmmoConsumption = 5
			self.Primary_TFA.NumShots = 5
	        self.Primary_TFA.Spread	= .02
			self:ClearStatCache()
		end
	end
...
if self:GetSilenced() and self:Clip1() <= 4 then
...
if self:GetSilenced() and self:Clip1() <= 3 then
