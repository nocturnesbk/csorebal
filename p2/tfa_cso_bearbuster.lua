...
SWEP.PrintName				= "Bear Buster"		-- Weapon name (Shown on HUD)	
...
SWEP.NZPaPName				= "Angry Russian Bear"
...
SWEP.ProjectileEntity = "bearbuster" --Entity to shoot
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_bearbuster.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_bearbuster_right.mdl" -- Worldmodel path
...
SWEP.WElements = {
    ["bearbuster_l"] = { type = "Model", model = "models/weapons/tfa_cso/w_bearbuster_left.mdl", bone = "ValveBiped.Bip01_L_Hand", rel = "", pos = Vector(0, -1, 0), angle = Angle(0, 85, 0), size = Vector(1.2, 1.2, 1.2), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
SWEP.Secondary.Sound = Sound("BearBuster.Fire2")
SWEP.Secondary.ClipSize			= -1					-- Size of a clip
SWEP.Secondary.DefaultClip			= 0					-- Default ammo to give...
SWEP.Secondary.Automatic			= false					-- Automatic/Semi Auto
SWEP.Secondary.Ammo			= "compressed_breath" -- Self explanitory, ammo type.
...
function SWEP:SecondaryAttack( ... )
	if CurTime() >  self:GetNextPrimaryFire() and TFA.Enum.ReadyStatus[ self:GetStatus() ] and not self:GetSprinting() and self:Ammo2() > 0 then
		...
		if SERVER then
			...
			local dmginfo = CSO:DMGInfo(self.Owner, self, 84, Vector(0,0,0), DMG_SONIC, self:GetPos())
			CSO:DoRadiusAttack(self, dmginfo, 225, true, 3072, 2, false, nil, true)
		end
	end
...
function SWEP:PostPrimaryAttack()
	...
	if SERVER then
		if self.NextGen >= 10 and self:Ammo2() < 1 then
...
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_bearbuster")
