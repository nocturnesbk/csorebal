...
SWEP.PrintName				= "Bear Fury MK-1"		-- Weapon name (Shown on HUD)	
...
--Firing related
SWEP.Primary.Sound 			= Sound("BearFuryMK1.Fire")				-- This is the sound of the weapon, when you shoot.
...
SWEP.NZPaPName				= "Long Jump Module (Bear Edition)"
...
SWEP.ProjectileEntity = "bearfurymk1" --Entity to shoot
SWEP.ProjectileVelocity = 2000 --Entity to shoot's velocity
SWEP.ProjectileModel = "models/hunter/misc/sphere1x1.mdl" --Entity to shoot's model

--[[VIEWMODEL]]--

SWEP.ViewModel			= "models/weapons/tfa_cso/c_bearfurymk1.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_bearfurymk1_right.mdl" -- Worldmodel path
...
SWEP.WElements = {
    ["bearfurymk1_l"] = { type = "Model", model = "models/weapons/tfa_cso/w_bearfurymk1_left.mdl", bone = "ValveBiped.Bip01_L_Hand", rel = "", pos = Vector(0, -1, 0), angle = Angle(0, 85, 0), size = Vector(1.2, 1.2, 1.2), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} },
    ["jetpack"] = { type = "Model", model = "models/weapons/tfa_cso/w_bearfurymk1_jetpack.mdl", bone = "ValveBiped.Bip01_Spine2", rel = "", pos = Vector(2, 7, 0), angle = Angle(90, 90, 0), size = Vector(1.2, 1.2, 1.2), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
SWEP.Secondary.Sound = Sound("BearFuryMK1.Dash")
SWEP.Secondary.ClipSize			= -1					-- Size of a clip
SWEP.Secondary.DefaultClip			= 0					-- Default ammo to give...
SWEP.Secondary.Automatic			= false					-- Automatic/Semi Auto
SWEP.Secondary.Ammo			= "bearjumpmk1" -- Self explanitory, ammo type.
...
function SWEP:SecondaryAttack()
	if self:Ammo2() > 0 and TFA.Enum.ReadyStatus[ self:GetStatus() ] and self.Owner:OnGround() then
		self.Owner:SetVelocity(self.Owner:GetForward() * 550 + self.Owner:GetUp() * 300)
		...
		self.Owner:SetVelocity(self.Owner:GetForward() * 550 + self.Owner:GetUp() * 200)
		self.Weapon:SendWeaponAnim(ACT_VM_DRYFIRE)
		self.NextGen = 0
		self:TakeSecondaryAmmo(1)
		self:EmitSound(self:GetStat("Secondary.Sound"))
...
function SWEP:PostPrimaryAttack()
	...
	if SERVER then
		if self.NextGen >= 25 and self:Ammo2() < 2 then
			self.Owner:GiveAmmo(1, self.Secondary.Ammo, true)
			self.NextGen = 0
			self:EmitSound("BearBuster.Beep")
		end
	end
...
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_bearfurymk1")
