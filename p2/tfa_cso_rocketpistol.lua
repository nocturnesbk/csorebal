...
SWEP.PrintName				= "Twin Hawk"		-- Weapon name (Shown on HUD)	
...
SWEP.Primary.Damage		= (engine.ActiveGamemode() == "sandbox" and 45) or 90					-- Damage, in standard damage points.
...
--Range Related
SWEP.Primary.Range = 250 -- The distance the bullet can travel in source units.  Set to -1 to autodetect based on damage/rpm.
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_rocketpistol.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_rocketpistol_right.mdl" -- Worldmodel path
...
SWEP.WElements = {
    ["rocketpistol_l"] = { type = "Model", model = "models/weapons/tfa_cso/w_rocketpistol_left.mdl", bone = "ValveBiped.Bip01_L_Hand", rel = "", pos = Vector(0, 0, 1), angle = Angle(-25, 90, 0), size = Vector(1.2, 1.2, 1.2), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
SWEP.MuzzleFlashEffect = "cso_muz_rocketpistol" --Change to a string of your muzzle flash effect.  Copy/paste one of the existing from the base.
...
SWEP.Secondary.Ammo = "84mm_rocket"
SWEP.Secondary.Ent = "rocketpistol_rocket"
SWEP.Secondary.Automatic			= true
SWEP.Secondary.Model = "models/hunter/misc/sphere1x1.mdl" --Projectile Model
SWEP.Secondary.Damage = 500 --Grenade Damage
SWEP.Secondary.Velocity = 3000 --Grenade Velocity
SWEP.Secondary.Delay = 0.5 --Delay in seconds
SWEP.Secondary.Sound = Sound("RocketPistol.Fire2") --Firing sound
SWEP.Secondary.Recoil = Angle(-1,0,0) --Recoil Dir
...
local Seq = VModel:LookupSequence("shoot21")
local Seq2 = VModel:LookupSequence("shoot22")
	...
	self:GetOwner():SetAnimation(PLAYER_ATTACK1)
		...
		if self:Ammo2()%10 == 9 or self:Ammo2()%10 == 7 or self:Ammo2()%10 == 5 or self:Ammo2()%10 == 3 or self:Ammo2()%10 == 1 then 
			VModel:SendViewModelMatchingSequence( Seq )
			self:SetNextIdleAnim(CurTime() + 0.8)
		end
		if  self:Ammo2()%10 == 8 or self:Ammo2()%10 == 6 or self:Ammo2()%10 == 4 or self:Ammo2()%10 == 2  or self:Ammo2()%10 == 0 then 
...
function SWEP:PostPrimaryAttack()
	...
	if SERVER then
		if self.NextGen >= 3 and self:Ammo2() < 10 then
...
function SWEP:Think2( ... )
	...
	if shooting ~= self.Shooting_Old then
		if shooting then
			self:EmitSound("RocketPistol.Fire_Loop")
			self.NextIdleSound = CurTime() + 0
		else
			self:EmitSound("RocketPistol.Fire_Loop_End")
	...
	if shooting then
		if self.NextIdleSound and CurTime() > self.NextIdleSound then
			self:EmitSound("RocketPistol.Fire_Loop")
			self.NextIdleSound = CurTime() + SoundDuration( "RocketPistol.Fire_Loop" ) - 0.1
		end
	end
	...
	if self:GetStatus() == TFA.GetStatus("shooting") then
	   if SERVER then
		  ...
		  util.Effect("exp_rocketpistol_ring", effectdata)
		end
	end
...
local range
local bul = {}
function SWEP:ShootBullet(dmginfo)
...
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_rocketpistol")
