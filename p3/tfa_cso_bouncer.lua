...
SWEP.Author				= "Kamikaze & Meirii" --Author Tooltip
...
SWEP.ProjectileEntity = "cso_dummyEntity" --Entity to shoot
...
if(SERVER) then
	hook.Add("EntityTakeDamage", "CSO_EntityTakeDamage", function(target, dmginfo)
		...
		if(attacker:GetClass() != "bouncer_pellets") then return end
		if(!IsValid(attacker:GetOwner()) || !IsValid(attacker:GetOwner():GetActiveWeapon())) then return end
		local d = dmginfo
	...
	function SWEP:PostPrimaryAttack()
		...
		for i = 1, self.Primary.NumShots, 1 do
			local _ang = Angle(math.random(-self.EntitySpread, self.EntitySpread), math.random(-self.EntitySpread, self.EntitySpread), 0)
			local pellet = ents.Create("bouncer_pellets")
				pellet:SetPos(ply:EyePos())
				pellet:SetAngles(ang + _ang)
				pellet:SetOwner(ply)
				pellet:Spawn()
				pellet.HitDamage = self.Primary.Damage
		end
	end
end