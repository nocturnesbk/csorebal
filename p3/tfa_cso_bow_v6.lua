...
SWEP.PrintName				= "Compound Bow Expert"		-- Weapon name (Shown on HUD)
...
SWEP.Primary.Damage		= 95 --effective damage				-- Damage, in standard damage points.
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_bow_v6.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_compound_v6.mdl" -- Weapon world model path
...
SWEP.TracerName 		= "tra_bow_v6" 	--Change to a string of your tracer name.  Can be custom.
...
function SWEP:Think2(...)
	if self:GetStatus() == TFA.Enum.STATUS_SILENCER_TOGGLE and self:GetStatusEnd() <= CurTime() then
		...
		if self:GetSilenced() then
			...
			self.Primary_TFA.Damage = 950
			self.Primary_TFA.RPM = 25
			self.Primary_TFA.AmmoConsumption = 1
			self.Primary_TFA.NumShots = 1
			self.MuzzleFlashEffectSilenced = ""
			self.TracerName = "tra_bow_v6_charged"
