...
SWEP.Primary.Damage_Secondary		= 105					-- Damage, in standard damage points.

--Impact Effects
SWEP.ImpactEffect = "tfa_metal_impact"--Impact Effect
SWEP.ImpactDecal = "" --Impact Decal
SWEP.Primary.ImpactEffect = "exp_broad"
...
function SWEP:PostPrimaryAttack() -- GLOBAL HERE, UHHHH
	if self:Clip1() == 191 then
		self.Primary_TFA.Damage = self.Primary.Damage_Secondary
		self.Primary_TFA.DamageType = bit.bor(DMG_BLAST,DMG_AIRBOAT,DMG_BURN,DMG_ALWAYSGIB)
	end
	if self:Clip1() == 190 then
		self.Primary_TFA.Damage = self.Primary.Damage_Regular
		self.Primary_TFA.DamageType = DMG_BULLET
	end
	if self:Clip1() == 181 then
		self.Primary_TFA.Damage = self.Primary.Damage_Secondary
		self.Primary_TFA.DamageType = bit.bor(DMG_BLAST,DMG_AIRBOAT,DMG_BURN,DMG_ALWAYSGIB)
	end
	if self:Clip1() == 180 then
		self.Primary_TFA.Damage = self.Primary.Damage_Regular
		self.Primary_TFA.DamageType = DMG_BULLET
	end
	if self:Clip1() == 171 then
		self.Primary_TFA.Damage = self.Primary.Damage_Secondary
		self.Primary_TFA.DamageType = bit.bor(DMG_BLAST,DMG_AIRBOAT,DMG_BURN,DMG_ALWAYSGIB)
	end
	if self:Clip1() == 170 then
		self.Primary_TFA.Damage = self.Primary.Damage_Regular
		self.Primary_TFA.DamageType = DMG_BULLET
	end
	if self:Clip1() == 161 then
		self.Primary_TFA.Damage = self.Primary.Damage_Secondary
		self.Primary_TFA.DamageType = bit.bor(DMG_BLAST,DMG_AIRBOAT,DMG_BURN,DMG_ALWAYSGIB)
	end
	if self:Clip1() == 160 then
		self.Primary_TFA.Damage = self.Primary.Damage_Regular
		self.Primary_TFA.DamageType = DMG_BULLET
	end
	if self:Clip1() == 151 then
		self.Primary_TFA.Damage = self.Primary.Damage_Secondary
		self.Primary_TFA.DamageType = bit.bor(DMG_BLAST,DMG_AIRBOAT,DMG_BURN,DMG_ALWAYSGIB)
	end
	if self:Clip1() == 150 then
		self.Primary_TFA.Damage = self.Primary.Damage_Regular
		self.Primary_TFA.DamageType = DMG_BULLET
	end
	if self:Clip1() == 141 then
		self.Primary_TFA.Damage = self.Primary.Damage_Secondary
		self.Primary_TFA.DamageType = bit.bor(DMG_BLAST,DMG_AIRBOAT,DMG_BURN,DMG_ALWAYSGIB)
	end
	if self:Clip1() == 140 then
		self.Primary_TFA.Damage = self.Primary.Damage_Regular
		self.Primary_TFA.DamageType = DMG_BULLET
	end
	if self:Clip1() == 131 then
		self.Primary_TFA.Damage = self.Primary.Damage_Secondary
		self.Primary_TFA.DamageType = bit.bor(DMG_BLAST,DMG_AIRBOAT,DMG_BURN,DMG_ALWAYSGIB)
	end
	if self:Clip1() == 130 then
		self.Primary_TFA.Damage = self.Primary.Damage_Regular
		self.Primary_TFA.DamageType = DMG_BULLET
	end
	if self:Clip1() == 121 then
		self.Primary_TFA.Damage = self.Primary.Damage_Secondary
		self.Primary_TFA.DamageType = bit.bor(DMG_BLAST,DMG_AIRBOAT,DMG_BURN,DMG_ALWAYSGIB)
	end
	...
	if self:Clip1() == 111 then
		self.Primary_TFA.Damage = self.Primary.Damage_Secondary
		self.Primary_TFA.DamageType = bit.bor(DMG_BLAST,DMG_AIRBOAT,DMG_BURN,DMG_ALWAYSGIB)
	end
	if self:Clip1() == 110 then
		self.Primary_TFA.Damage = self.Primary.Damage_Regular
		self.Primary_TFA.DamageType = DMG_BULLET
	end
	if self:Clip1() == 101 then
		self.Primary_TFA.Damage = self.Primary.Damage_Secondary
		self.Primary_TFA.DamageType = bit.bor(DMG_BLAST,DMG_AIRBOAT,DMG_BURN,DMG_ALWAYSGIB)
	end
	...
	if self:Clip1() == 0 or self:Clip1() == 199 then
		self.Primary_TFA.Damage = self.Primary.Damage_Regular
		self.Primary_TFA.DamageType = DMG_BULLET
	end
