...
SWEP.MuzzleFlashEffect = "cso_muz_gungnir" --Change to a string of your muzzle flash effect.  Copy/paste one of the existing from the base.

--Tracer Stuff

SWEP.Tracer				= 0		--Bullet tracer.  TracerName overrides this.
SWEP.TracerName 		= "cso_tra_gungnir_a" 	--Change to a string of your tracer name.  Can be custom.
...
function SWEP:Think2(...)
	if self:GetStatus() == TFA.Enum.STATUS_SILENCER_TOGGLE and self:GetStatusEnd() <= CurTime() then
		...
		if self:GetSilenced() then
			...
			self.MuzzleFlashEffect = "cso_muz_gungnir"
			self.TracerName = "cso_tra_gungnir_a"
			self.HoldType = "shotgun"
			self.WorldModel	= "models/weapons/tfa_cso/w_gungnira.mdl"
			self.Offset = {
			Pos = {
			Up = -6,
			Right = 3,
			...
			self.TracerName = "tra_gungnir"
			self.HoldType = "grenade"
			self.WorldModel	= "models/weapons/tfa_cso/w_gungnirb.mdl"
			self.Offset = {
			Pos = {
			Up = 0,
			Right = 20,
