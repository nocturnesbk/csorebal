...
SWEP.Purpose				= "Right-click to transform into Ruin Strike. Hit suit zoom for a projectile attack." --Purpose Tooltip
...
SWEP.PrintName				= "Dark Star"		-- Weapon name (Shown on HUD)	
...
SWEP.Primary.SilencedSound			= Sound("Gungnir.Shoot_B")				-- This is the sound of the weapon, when you shoot.
...
SWEP.MoveSpeed = 0.98 --Multiply the player's movespeed by this.
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_gungnirex.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_gungnirex.mdl" -- Worldmodel path
...
SWEP.MuzzleFlashEffect = "cso_muz_gungnirex" --Change to a string of your muzzle flash effect.  Copy/paste one of the existing from the base.

--Tracer Stuff

SWEP.Tracer				= 0		--Bullet tracer.  TracerName overrides this.
SWEP.TracerName 		= "cso_tra_gungnirex_a" 	--Change to a string of your tracer name.  Can be custom.
...
SWEP.Primary.ImpactEffect = "exp_gungnirex"
...
SWEP.Base				= "tfa_gun_base"
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_gungnirex")
	SWEP.DrawWeaponSelection = TFA_CSO_DrawWeaponSelection
end

DEFINE_BASECLASS("tfa_gun_base")

SWEP.Secondary.Ent = "gungnirexb"
SWEP.Secondary.Velocity = 5000
SWEP.Secondary.Automatic = true
SWEP.Secondary.ClipSize = -1
SWEP.Secondary.Delay = 0.8
SWEP.Secondary.Sound = Sound("Gungnir.Shoot_B")
SWEP.Secondary.Recoil = Angle(-1,0,0) --Recoil Dir

function SWEP:AltAttack()
	if CurTime() >  self:GetNextSecondaryFire() and TFA.Enum.ReadyStatus[ self:GetStatus() ] and not self:GetSprinting() and self:Clip1() > 0 and not self:GetSilenced() then
...
function SWEP:Think2(...)
	if self:GetStatus() == TFA.Enum.STATUS_SILENCER_TOGGLE and self:GetStatusEnd() <= CurTime() then
		...
		if self:GetSilenced() then
			...
			self.MuzzleFlashEffect = "cso_muz_gungnirex"
			self.TracerName = "cso_tra_gungnirex_a"
			self.HoldType = "shotgun"
			self.WorldModel	= "models/weapons/tfa_cso/w_gungnirex.mdl"
			self.Offset = {
			Pos = {
			Up = -6,
			Right = 3,
			...
			self.Primary_TFA.Damage = 52500	
			self.Primary_TFA.DamageType = DMG_BLAST
			self.Primary_TFA.BlastRadius	= 650
			self.Primary_TFA.RPM = 20
			self.Primary_TFA.AmmoConsumption = 7
			self.Primary_TFA.NumShots = 1
			self.MuzzleFlashEffectSilenced = nil
			self.TracerName = "tra_gungnirex"
			self.HoldType = "grenade"
			self.WorldModel	= "models/weapons/tfa_cso/w_gungnirexb.mdl"
			self.Offset = {
			Pos = {
			Up = 0,
			Right = 20,
