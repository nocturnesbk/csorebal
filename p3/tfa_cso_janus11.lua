...
SWEP.Purpose				= "Firing this weapon builds a secondary charge. Once you've fired 15 or more times, right-click to activate JANUS mode. Fire indefinitely until 8 seconds pass." --Purpose Tooltip
SWEP.Instructions				= "" --Instructions Tooltip
SWEP.Spawnable				= true --Can you, as a normal user, spawn this?
SWEP.AdminSpawnable			= true --Can an adminstrator spawn this?  Does not tie into your admin mod necessarily, unless its coded to allow for GMod's default ranks somewhere in its code.  Evolve and ULX should work, but try to use weapon restriction rather than these.
SWEP.DrawCrosshair			= true		-- Draw the crosshair?
SWEP.PrintName				= "JANUS-11"		-- Weapon name (Shown on HUD)
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_janus11.mdl" --Viewmodel path
...
SWEP.Base				= "tfa_gun_base"
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_janus11")
...
function SWEP:Think2(...)
	if self:GetStatus() == TFA.Enum.STATUS_SILENCER_TOGGLE and self:GetStatusEnd() <= CurTime() then
		...
		if self:GetSilenced() then
			self.Primary_TFA.Damage = 17
			self.Primary_TFA.Sound = Sound("JANUS11.Fire1")
			self.Primary_TFA.RPM = 90
			...
			self.Primary_TFA.Damage = 51
			self.Primary_TFA.Sound = Sound("JANUS11.Fire2")
			self.Primary_TFA.RPM = 150
...
SWEP.Secondary.Ammo			= "janus_11" -- Self explanitory, ammo type.
