...
SWEP.Purpose				= "Firing this weapon builds a secondary charge. Once you've fired 60 or more times, right-click to activate JANUS mode. Fire indefinitely until 14 seconds pass." --Purpose Tooltip
SWEP.Instructions				= "" --Instructions Tooltip
SWEP.Spawnable				= true --Can you, as a normal user, spawn this?
SWEP.AdminSpawnable			= true --Can an adminstrator spawn this?  Does not tie into your admin mod necessarily, unless its coded to allow for GMod's default ranks somewhere in its code.  Evolve and ULX should work, but try to use weapon restriction rather than these.
SWEP.DrawCrosshair			= true		-- Draw the crosshair?
SWEP.PrintName				= "JANUS-5"		-- Weapon name (Shown on HUD)
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_janus5.mdl" --Viewmodel path
...
SWEP.Base				= "tfa_gun_base"
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_janus5")
...
function SWEP:Think2(...)
	if self:GetStatus() == TFA.Enum.STATUS_SILENCER_TOGGLE and self:GetStatusEnd() <= CurTime() then
		...
		if self:GetSilenced() then
			self.Primary_TFA.Damage = 30
			self.Primary_TFA.Sound = Sound("Janus5.FireA")
			...
			self.Primary_TFA.Damage = 20
			self.Primary_TFA.Sound = Sound("Janus5.Fire2")
			self.Primary_TFA.RPM = 950
			self.Primary_TFA.AmmoConsumption = 0
			self.Primary_TFA.NumShots = 3
			self.Primary_TFA.Spread		= .009
...
SWEP.Secondary.Ammo			= "janus_5" -- Self explanitory, ammo type.

function SWEP:PostPrimaryAttack()
self.NextGen = self.NextGen +1
if SERVER then
if self.NextGen >= 1 and not self:GetSilenced() and self:Ammo2() < 240 then
self.Owner:GiveAmmo( 4,self.Secondary.Ammo, true )           
self.NextGen  = 0
end
if self:GetSilenced() then
