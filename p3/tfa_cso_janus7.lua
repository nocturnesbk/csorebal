...
SWEP.Purpose				= "Firing this weapon builds a secondary charge. Once you've fired 120 or more times, right-click to activate JANUS mode. Fire indefinitely until 18 seconds pass." --Purpose Tooltip
SWEP.Instructions				= "" --Instructions Tooltip
SWEP.Spawnable				= true --Can you, as a normal user, spawn this?
SWEP.AdminSpawnable			= true --Can an adminstrator spawn this?  Does not tie into your admin mod necessarily, unless its coded to allow for GMod's default ranks somewhere in its code.  Evolve and ULX should work, but try to use weapon restriction rather than these.
SWEP.DrawCrosshair			= true		-- Draw the crosshair?
SWEP.PrintName				= "JANUS-7"		-- Weapon name (Shown on HUD)
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_janus7.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_janus7.mdl" -- Worldmodel path
...
SWEP.Base				= "tfa_gun_base"
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_janus7")
...
function SWEP:SecondaryAttack()
    if self:Ammo2() < 240 then return end
...
function SWEP:Think2(...)
	if self:GetStatus() == TFA.Enum.STATUS_SILENCER_TOGGLE and self:GetStatusEnd() <= CurTime() then
		...
		if self:GetSilenced() then
			...
			self.TracerName = "cso_tra_janus7b"
		    self:ClearStatCache()
		end
	end
...
if SERVER then
if self.NextGen >= 1 and not self:GetSilenced() and self:Ammo2() < 240 then
...
if self:Ammo2() == 240 and self.NextGen == 0 then
