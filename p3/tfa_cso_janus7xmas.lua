...
SWEP.Purpose				= "Firing this weapon builds a secondary charge. Once you've fired 100 or more times, right-click to activate JANUS mode. Fire indefinitely until 18 seconds pass." --Purpose Tooltip
SWEP.Instructions				= "" --Instructions Tooltip
SWEP.Spawnable				= true --Can you, as a normal user, spawn this?
SWEP.AdminSpawnable			= true --Can an adminstrator spawn this?  Does not tie into your admin mod necessarily, unless its coded to allow for GMod's default ranks somewhere in its code.  Evolve and ULX should work, but try to use weapon restriction rather than these.
SWEP.DrawCrosshair			= true		-- Draw the crosshair?
SWEP.PrintName				= "JANUS-7 Xmas"		-- Weapon name (Shown on HUD)
...
--Firing related
SWEP.Primary.Sound 			= Sound("Janus7.Fire1")	
SWEP.Primary.SilencedSound 	= Sound("Janus7.Fire2")				-- This is the sound of the weapon, when you shoot.
...
SWEP.NZPaPName				= "Angry Santa's Buzzsaw"
...
SWEP.Primary.Spread		= .00425					--This is hip-fire acuracy.  Less is more (1 is horribly awful, .0001 is close to perfect)
SWEP.Primary.IronAccuracy = .002125	-- Ironsight accuracy, should be the same for shotguns
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_janus7xmas.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_janus7xmas.mdl" -- Worldmodel path
...
SWEP.IronSightsPos = Vector(6.1, -0.5, 1.8)
SWEP.IronSightsAng = Vector(0.6, -1.209, -3.855)
...
SWEP.BlowbackVector = Vector(0.1,0,0.5) --Vector to move bone <or root> relative to bone <or view> orientation.
...
SWEP.Base				= "tfa_gun_base"
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_janus7xmas")
...
function SWEP:SecondaryAttack()
    if self:Ammo2() < 200 then return end
	...
	if SERVER or not sp then
		self:ChooseSilenceAnim( not self:GetSilenced() )
		self:SetStatus(TFA.Enum.STATUS_SILENCER_TOGGLE)
		self:SetStatusEnd(CurTime() + 1.75)
...
function SWEP:Think2(...)
	if self:GetStatus() == TFA.Enum.STATUS_SILENCER_TOGGLE and self:GetStatusEnd() <= CurTime() then
		...
		if self:GetSilenced() then
			...
			self.Primary_TFA.Spread = .00425
			self.Primary_TFA.KickUp = 0.18
			self.Primary_TFA.KickDown = 0.12
			self.Primary_TFA.KickHorizontal = 0.15
			self.MuzzleFlashEffect = "tfa_muzzleflash_generic"
			self.TracerName = nil
		    self:ClearStatCache()
		else
			self.Primary_TFA.Damage = 75
			self.Primary_TFA.DamageType = bit.bor (DMG_BULLET,DMG_DISSOLVE,DMG_AIRBOAT)
			self.Primary_TFA.RPM = 800
			self.Primary_TFA.AmmoConsumption = 0
			self.Primary_TFA.Spread = .0005
			self.Primary_TFA.KickUp = 0.02
			self.Primary_TFA.KickDown = 0.02
			self.Primary_TFA.KickHorizontal = 0.001
			self.Primary_TFA.NumShots = 1
			self.MuzzleFlashEffectSilenced = "none"
			self.TracerName = "cso_tra_janus7bxmas"
		    self:ClearStatCache()
		end
	end
...
SWEP.Secondary.Ammo			= "janus_7" -- Self explanitory, ammo type.
SWEP.Secondary.ClipSize = -1

function SWEP:PostPrimaryAttack()
self.NextGen = self.NextGen +1
if SERVER then
if self.NextGen >= 1 and not self:GetSilenced() and self:Ammo2() < 200 then
self.Owner:GiveAmmo( 2,self.Secondary.Ammo, true )           
self.NextGen  = 0
end
if self:GetSilenced() then
...
if self:Ammo2() == 200 and self.NextGen == 0 then
self:EmitSound("Janus.Ready")
end
