...
SWEP.Primary.Damage		= 158					-- Damage, in standard damage points.
...
function SWEP:Think2(...)
	if self:GetIronSightsProgress() <= 0.5 and self:GetCSO_SwitchState() then
		self.Primary_TFA.Damage = 158
		self.Primary_TFA.Sound = Sound("Paladin.Fire")
		self.TracerName = "cso_tra_pal"
		self.Primary_TFA.RPM = 600
		self.Primary_TFA.KickUp = 0.215
		self.Primary_TFA.KickDown = 0.115
		...
		self.Primary_TFA.ProjectileVelocity = 0
		self:ClearStatCache()
	elseif self:GetIronSightsProgress() > 0.5 and not self:GetCSO_SwitchState() then
		self.TracerName = nil
		self.Primary_TFA.Damage = 480
		self.Primary_TFA.Sound = Sound("Paladin.Fire2")
		self.Primary_TFA.RPM = 120
		self.Primary_TFA.KickUp = 0.4
		self.Primary_TFA.KickDown = 0.25
		self.Primary_TFA.KickHorizontal = 0.18
		self.Primary_TFA.AmmoConsumption = 1
		self.Primary_TFA.NumShots = 1
		self.MuzzleFlashEffect = "cso_muz_des_wm"
		self:SetCSO_SwitchState(true)
		self.Primary_TFA.Projectile = "buffak47_ball_v8"
		self.Primary_TFA.ProjectileVelocity = 8000
		self:ClearStatCache()
	end
