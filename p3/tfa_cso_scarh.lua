...
SWEP.Primary.SilencedSound = Sound("SCAR.Fire2") -- This is the sound of the weapon, when silenced.
SWEP.Primary.Damage_Regular		= 28					-- Damage, in standard damage points.
SWEP.Primary.Damage		= SWEP.Primary.Damage_Regular
SWEP.Primary.Damage_Secondary		= 34					-- Damage, in standard damage points.
...
function SWEP:Think2(...)
	if self:GetStatus() == TFA.Enum.STATUS_SILENCER_TOGGLE and self:GetStatusEnd() <= CurTime() then
		...
		if self:GetSilenced() then
			...
			self.WorldModel			= "models/weapons/tfa_cso/w_scarl.mdl"
			self.Offset = {
			Pos = {
			Up = -4.5,
			Right = 0.7,
			...
			self.WorldModel			= "models/weapons/tfa_cso/w_scarh.mdl"
			self.Offset = {
			Pos = {
			Up = -4.5,
			Right = 0.7,
