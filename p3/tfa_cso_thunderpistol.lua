...
SWEP.Secondary.DefaultClip			= 100					-- Default ammo to give...
SWEP.Secondary.Automatic			= true					-- Automatic/Semi Auto
SWEP.Secondary.Ammo			= "stealth" -- Self explanitory, ammo type.
SWEP.Secondary.AmmoConsume = 0
...
function SWEP:PrimaryAttack(...)
	...
	if TFA.Enum.ReadyStatus[ self:GetStatus() ] and not self:GetSprinting() then
		self:SetNWBool( "V_CloakOn", false )
		self:SetNWBool( "W_CloakOn", false )
		self.CloakOn = 0
...
function SWEP:Think(...) 
	if self:GetNWBool("V_CloakOn") == true then
		self.Owner:GetViewModel():SetMaterial("sprites/heatwave")
	else
		self.Owner:GetViewModel():SetMaterial("")	
	end
	if self:GetNWBool("W_CloakOn") == true and self:Ammo2() > 0 then
		self.Owner:SetMaterial("sprites/heatwave")
		self.Owner:DrawShadow( false )
		self.Owner:RemoveAllDecals()
		self.Primary_TFA.Damage = 2400
		self.Primary_TFA.ImpactEffect = "exp_thunderpistol_cloak"
		self.MuzzleFlashEffect = "cso_muz_thunderpistol_cloak"	
		self.TracerName = "tra_thunderpistol_cloak"
	else
		self.Owner:SetMaterial("")
		self.Owner:DrawShadow( true )
		self.Primary_TFA.Damage = 1200
		self.Primary_TFA.ImpactEffect = "exp_thunderpistol"
		self.MuzzleFlashEffect = "cso_muz_thunderpistol"
		self.TracerName = "tra_thunderpistol"		
	end
	if SERVER then
		if self.Secondary.AmmoConsume <= CurTime() and self:GetNWBool("W_CloakOn") == true then 
		self:TakeSecondaryAmmo( 1,"stealth", true )             -- Secondary ammo consume
		self.Secondary.AmmoConsume =  CurTime()  + 0.075   -- Seconds to consume
		self.Secondary.Nextregen =  CurTime()  + 10   ---6 seconds
	else
		if self.Secondary.Nextregen <= CurTime() and self:Ammo2() < 100 and self:GetNWBool("W_CloakOn") == false then 
		self.Owner:GiveAmmo( 10,"stealth", true )                     -------SECONDARY AMMO REGEN
		self.Secondary.Nextregen =  CurTime()  + 10   ---10 seconds
	end
end
end
	...
	if self:GetOwner():KeyDown(IN_ATTACK) and not self:GetSprinting() and self:GetNWBool("W_CloakOn") == true and TFA.Enum.ReadyStatus[ self:GetStatus() ] then
...
function SWEP:SecondaryAttack()
	if TFA.Enum.ReadyStatus[ self:GetStatus() ] and self.CloakOn == 0 and self:Ammo2() > 0 then
		self:SetNWBool("V_CloakOn", true)
		self:SetNWBool("W_CloakOn", true)
		self.CloakOn = 1
		self:SetNextSecondaryFire(CurTime() + 0.5)
		self:EmitSound("Thunder.Cloak")
		if SERVER then
