...
SWEP.PrintName				= "Death Eater"		-- Weapon name (Shown on HUD)
...
SWEP.Type 	= 	"Unique Grade Shotgun"
...
--Firing related
SWEP.Primary.Sound 			= Sound("DeathEater.Fire")				-- This is the sound of the weapon, when you shoot.
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_death_eater.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_death_eater.mdl" -- Worldmodel path
...
SWEP.MuzzleFlashEffect = "cso_muz_de" --Change to a string of your muzzle flash effect.  Copy/paste one of the existing from the base.

--Tracer Stuff

SWEP.Tracer				= 0		--Bullet tracer.  TracerName overrides this.
SWEP.TracerName 		= "cso_tra_de" 	--Change to a string of your tracer name.  Can be custom.
...
SWEP.Base				= "tfa_gun_base"
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_death_eater")
...
SWEP.LaserAnimation = {
	["start"] = {
		...
		["value"] = "shoot2_start", --Number for act, String/Number for sequence
		["transition"] = true
	}, --Inward transition
	["loop"] = {
		["type"] = TFA.Enum.ANIMATION_SEQ, --Sequence or act
		["value"] = "shoot2", --Number for act, String/Number for sequence
		["is_idle"] = true
	},--looping animation
	["end"] = {
		["type"] = TFA.Enum.ANIMATION_SEQ, --Sequence or act
		["value"] = "shoot2_end", --Number for act, String/Number for sequence
		["transition"] = true
	} --Outward transition
}

SWEP.LaserSound_Idle = ""
SWEP.LaserSound_Loop = "DeathEater.Fire2"
SWEP.LaserSound_Start = "DeathEater.Fire_Laser"
SWEP.LaserSound_End = "DeathEater.Fire2_End"

SWEP.LaserSound_Idle_Next = -1
SWEP.LaserSound_Loop_Next = -1

SWEP.LaserSound_BlendTime = 0.05

SWEP.LaserDrain_Idle = 0
SWEP.LaserDrain_Loop = 7--Ammo per second


SWEP.Secondary.MaxAmmo = 25
SWEP.Secondary.Reach = 1500
SWEP.Secondary.RPM = 600
SWEP.Secondary.ClipSize			= -1					-- Size of a clip
SWEP.Secondary.DefaultClip			= 0					-- Default ammo to give...
SWEP.Secondary.Damage = 750
SWEP.Secondary.NumShots = 1
SWEP.Secondary.Automatic			= true					-- Automatic/Semi Auto
SWEP.Secondary.Ammo			= "laser_beam" -- Self explanitory, ammo type.
...
function SWEP:LaserThink()
	...
	if self:Ammo2() > 0 and self.Owner:KeyDown(IN_ATTACK2) and TFA.Enum.ReadyStatus[stat] and not self:GetSprinting() then
		...
		if self:CanSoundPredict() then
			self:EmitSound(self.LaserSound_Start)
	...
	elseif stat == TFA.GetStatus("laser_attack") and ( ( not self.Owner:KeyDown(IN_ATTACK2) ) or self:Ammo2() <= 0 or self:GetSprinting() ) then
		...
		self:SetStatusEnd( CurTime() + 0.45 )
		if self:CanSoundPredict() then
			self:EmitSound(self.LaserSound_End)
			self.LaserSound_Idle_Next = SysTime() + 0.1
		end
...
function SWEP:LaserSounds()
	...
	if self:GetStatus() == TFA.GetStatus("laser_attack") then
		...
		if self.LaserSound_Idle_Next ~= -1 then
			self:StopSound( self.LaserSound_Idle )
			self.LaserSound_Idle_Next = -1
		end
	elseif self:Ammo2() >= 0 and stat ~= TFA.GetStatus("reloading") then
		if SysTime() > self.LaserSound_Idle_Next then
			self.LaserSound_Idle_Next = SysTime() + SoundDuration( self.LaserSound_Idle ) / self:GetTimeScale() - self.LaserSound_BlendTime
			self:EmitSound( self.LaserSound_Idle )
		end
		if self.LaserSound_Loop_Next ~= -1 then
			self:StopSound( self.LaserSound_Loop )
			self.LaserSound_Loop_Next = -1
		end
	end
end
	...
	if lasertrace.Hit then
		if game.GetTimeScale() > 0.99 then
			self.Owner:FireBullets({
				...
				HullSize = 12.5,
				Tracer = 1,
				TracerName = "tra_death_eater",
				Src = self.Owner:GetShootPos(),
				Dir = lasertrace.Normal,
				Callback = function(a, b, c)
					c:SetDamageType( bit.bor( DMG_DISSOLVE,DMG_ALWAYSGIB) )
				end
			})
		else
			...
			dmg:SetDamageForce(self.Owner:GetAimVector() * (750))
			dmg:SetDamage(750)
...
function SWEP:Think() 
if SERVER then
if self.Secondary.Nextregen <= CurTime()  and self:Ammo2() < 25 then 
self.Owner:GiveAmmo( 1,"laser_beam", true )                     -------SECONDARY AMMO REGEN
self.Secondary.Nextregen =  CurTime()  + 1
end
end
