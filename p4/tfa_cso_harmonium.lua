...
SWEP.PrintName				= "Psychic Harmonium"		-- Weapon name (Shown on HUD)
...
--Firing related
SWEP.Primary.Sound 			= Sound("Harmonium.Fire1")				-- This is the sound of the weapon, when you shoot.
...
SWEP.ProjectileEntity = "pianogun" --Entity to shoot
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_harmonium.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_harmonium_right.mdl" -- Worldmodel path
...
SWEP.WElements = {
	["other_pianogun"] = { type = "Model", model = "models/weapons/tfa_cso/w_harmonium.mdl", bone = "ValveBiped.Bip01_L_Hand", rel = "", pos = Vector(6.479, -4, 2.755), angle = Angle(0, 75.515, 0), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
SWEP.MuzzleFlashEffect = "cso_muz_pianogun_a" --Change to a string of your muzzle flash effect.  Copy/paste one of the existing from the base.
...
SWEP.VElements = {
	["reload"] = { type = "Model", model = "models/hunter/plates/plate1x1.mdl", bone = "Point_crosshair", rel = "", pos = Vector(0, 0, 0), angle = Angle(0, 0, 0), size = Vector(0.25, 0.25, 0), color = Color(255, 255, 255, 255), surpresslightning = false, material = "sprites/ef_pianogun_reload01", skin = 0, bodygroup = {}, active = false },
	["b_mode"] = { type = "Model", model = "models/hunter/plates/plate1x1.mdl", bone = "Point_crosshair", rel = "", pos = Vector(0, 0, 0), angle = Angle(0, 0, 0), size = Vector(0.25, 0.25, 0), color = Color(255, 255, 255, 255), surpresslightning = false, material = "sprites/ef_pianogun_muzzle02", skin = 0, bodygroup = {}, active = false },
}


SWEP.EventTable = {
    [ACT_VM_RELOAD] = {
        {time = 2 / 30, type = "lua", value = function(wep)
            wep.VElements.reload.active = true
            wep:ClearStatCache("VElements.reload.active")
        end}
    },
    [ACT_VM_IDLE] = {
        {time = 1 / 30, type = "lua", value = function(wep)
            wep.VElements.reload.active = false
            wep:ClearStatCache("VElements.reload.active")
        end}
    },
    [ACT_VM_DRAW] = {
        {time = 1 / 30, type = "lua", value = function(wep)
            wep.VElements.reload.active = false
            wep:ClearStatCache("VVElements.reload.active")
            wep.VElements.reload.active = false
            wep:ClearStatCache("VElements.reload.active")
        end}
    }
...
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_harmonium")
...
function SWEP:Holster( ... )
	self:StopSound("Pianogunex.Idle")
...
function SWEP:SecondaryAttack(...)
   if self:Clip1() <= 11 then return end
   ...
   self:SetNextIdleAnim(CurTime() + 1.5)
   self:TakePrimaryAmmo(12)
   self.Weapon:SendWeaponAnim(ACT_VM_SECONDARYATTACK)
   self:GetOwner():SetAnimation(PLAYER_ATTACK1)
   self:EmitSound("Harmonium.Fire2")

   if self.combo < self.comboright + self.comboleft then
       self.combo = self.combo + 1
       if SERVER then
        local dmginfo = CSO:DMGInfo(self.Owner, self, 1500, Vector(0,0,0), DMG_BLAST, self:GetPos())
        CSO:DoRadiusAttack(self, dmginfo, 350, true, 2048, 1.5, false, nil, true)
       end
       if IsFirstTimePredicted() then
         local effectdata = EffectData()
         effectdata:SetOrigin(self.Owner:GetPos())
         util.Effect("exp_harmonium", effectdata) -- easy effect
       end
   elseif self.combo >= self.comboright + self.comboleft then
       self.combo = 0
       self.Owner:SetHealth(math.min(self.Owner:Health() + self.Owner:GetMaxHealth() * 0.3), self.Owner:GetMaxHealth())
       if SERVER then
        local dmginfo = CSO:DMGInfo(self.Owner, self, 1500, Vector(0,0,0), DMG_BLAST, self:GetPos())
        CSO:DoRadiusAttack(self, dmginfo, 375, true, 2048, 2, false, nil, true)
       end
       if IsFirstTimePredicted() then
         local effectdata = EffectData()
         effectdata:SetOrigin(self.Owner:GetPos())
...
hook.Add( "PlayerShouldTakeDamage", "antiselfdamage_pianogun", function( ply, attacker ) -- I want to remove this hook but projectiles will self-damage
   if ply == attacker and (ply:GetActiveWeapon():GetClass() == "tfa_cso_harmonium") then -- grumble grumble
      return false
   end
end )