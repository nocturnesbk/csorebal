...
SWEP.Purpose				= "Firing this weapon builds a secondary charge. Once you've fired 5 or more times, right-click to activate JANUS mode. Fire indefinitely until 7 seconds pass." --Purpose Tooltip
SWEP.Instructions				= "" --Instructions Tooltip
SWEP.Spawnable				= true --Can you, as a normal user, spawn this?
SWEP.AdminSpawnable			= true --Can an adminstrator spawn this?  Does not tie into your admin mod necessarily, unless its coded to allow for GMod's default ranks somewhere in its code.  Evolve and ULX should work, but try to use weapon restriction rather than these.
SWEP.DrawCrosshair			= true		-- Draw the crosshair?
SWEP.PrintName				= "JANUS-1"		-- Weapon name (Shown on HUD)
...
--Firing related
SWEP.Primary.Sound 			= Sound("Janus1.Fire")	
SWEP.Primary.SilencedSound	= Sound("Janus1.Fire2")			-- This is the sound of the weapon, when you shoot.
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_janus1.mdl" --Viewmodel path
...
SWEP.Base				= "tfa_gun_base"
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_janus1")
...
function SWEP:Initialize()
	...
	self.StatCache_Blacklist["Primary.ProjectileVelocity"] = true

	self:ClearStatCache()
end

function SWEP:SecondaryAttack()
	if self:Ammo2() < 15 then return end
	if self:GetSilenced() then return end
	if not self:CanPrimaryAttack() and self:GetStatus() == TFA.Enum.STATUS_SILENCER_TOGGLE then return end
	timer.Simple(7, function()
		if self:GetSilenced() or self:Ammo2() < 0 then
		self:ChooseSilenceAnim( not self:GetSilenced() )
		self:SetStatus(TFA.Enum.STATUS_SILENCER_TOGGLE)
		self:SetStatusEnd(CurTime() + 1.25)
...
function SWEP:Think2(...)
	if self:GetStatus() == TFA.Enum.STATUS_SILENCER_TOGGLE and self:GetStatusEnd() <= CurTime() then
		...
		if self:GetSilenced() then
			...
			self.Primary_TFA.RPM = 22
			self.Primary_TFA.AmmoConsumption = 1
			self.Primary_TFA.NumShots = 1
			self.Primary_TFA.Projectile = "janus1a_nade"
			self.Primary_TFA.ProjectileVelocity = 1500
			...
			self.Primary_TFA.NumShots = 1
			self.Primary_TFA.Projectile = "janus1b_nade"
			self.Primary_TFA.ProjectileVelocity = 1500
...
SWEP.Secondary.Ammo			= "janus_1" -- Self explanitory, ammo type.

function SWEP:PostPrimaryAttack()
self.NextGen = self.NextGen +1
if SERVER then
if self.NextGen >= 1 and not self:GetSilenced() and self:Ammo2() < 15 then
...
if self:Ammo2() == 15 and self.NextGen == 0 then
