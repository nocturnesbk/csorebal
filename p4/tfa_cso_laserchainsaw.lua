...
SWEP.PrintName = "Laser Ripper"
SWEP.Type	= "Transcendent Grade Equipment"
SWEP.ViewModel = "models/weapons/tfa_cso/c_laserchainsaw.mdl"
...
SWEP.WorldModel = "models/weapons/tfa_cso/w_laserchainsaw.mdl"
SWEP.Offset = {
	Pos = {
		Up = 2,
		Right = 1.5,
...
SWEP.Saw_Sound_Idle = ""
SWEP.Saw_Sound_Saw = "LaserChainsaw.Attack_Loop"
SWEP.Saw_Sound_In = "LaserChainsaw.Attack_Start"
SWEP.Saw_Sound_Out = "LaserChainsaw.Attack_End"
...
SWEP.Secondary.BashHitSound = Sound("LaserChainsaw.HitWall")
SWEP.Secondary.BashHitSound_Flesh = Sound("LaserChainsaw.HitFleshSlash"..math.random(1,2))
...
function SWEP:SawThink()
	...
	if self:Clip1() == 0 then
		self.Secondary.BashHitSound = Sound("LaserChainsaw.Slash"..math.random(3,4))
		self.Secondary.BashHitSound_Flesh = Sound("LaserChainsaw.HitFleshSlash"..math.random(3,4))
		self.Secondary.BashDamage = 675
	end
	self:SawMechanics()
	self:SawSounds()
	self:AnimateSaw()
...
function SWEP:MuteSounds( offsnd )
	if self.Saw_Sound_Saw_Next ~= -1 then
		...
		self.Saw_Sound_Saw_Next = -1
		if offsnd then
			self:EmitSound("LaserChainsaw.Attack_End")
		end
	end
	if self.Saw_Sound_Idle_Next ~= -1 then
		self:StopSound( self.Saw_Sound_Idle )
		self.Saw_Sound_Idle_Next = -1
		if offsnd then
			self:EmitSound("LaserChainsaw.Idle")
		end
	end
end
...
function SWEP:Saw( damage, force, reach )
	...
	if slashtrace.Hit then
		if game.GetTimeScale() > 0.99 then
			...
			dmg:SetDamageForce(self.Owner:GetAimVector() * (80))
			dmg:SetDamage(80)
...
function SWEP:Holster( ... )
	self:StopSound("LaserChainsaw.Idle")
	self:StopSound("LaserChainsaw.Attack_Loop")
	return BaseClass.Holster(self,...)
end
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_laserchainsaw")
