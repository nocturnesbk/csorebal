...
SWEP.Purpose				= "Firing this weapon builds a secondary charge. Once you've fired 25 or more times, right-click to activate Transform System. You'll fire faster, deal more damage, and every 5th shot will be explosive." --Purpose Tooltip
SWEP.Instructions				= "" --Instructions Tooltip
SWEP.Spawnable				= true --Can you, as a normal user, spawn this?
SWEP.AdminSpawnable			= true --Can an adminstrator spawn this?  Does not tie into your admin mod necessarily, unless its coded to allow for GMod's default ranks somewhere in its code.  Evolve and ULX should work, but try to use weapon restriction rather than these.
SWEP.DrawCrosshair			= true		-- Draw the crosshair?
SWEP.PrintName				= "Mechasaurus MK-4"		-- Weapon name (Shown on HUD)	
...
SWEP.Type	= "Unique Grade Assault Rifle"
SWEP.ProceduralHolsterTime = 0

--[[WEAPON HANDLING]]--

--Firing related
SWEP.Primary.Sound 			= Sound("MechasaurusMK.Fire2")				-- This is the sound of the weapon, when you shoot.
SWEP.Primary.SilencedSound			= Sound("MechasaurusMK.Fire3")				-- This is the sound of the weapon, when you shoot.
SWEP.Primary.Damage_Regular		= 42					-- Damage for A-Mode.
SWEP.Primary.Damage		= SWEP.Primary.Damage_Regular
SWEP.Primary.Damage_Secondary		= 180					-- Damage, in standard damage points.
SWEP.Primary.Damage_BMode		= 55					-- Damage for B-Mode(not explosve rounds).
...
SWEP.NZPaPName				= "Jurassic-5 + Michael Bay"
...
SWEP.Primary.Spread		= .0050					--This is hip-fire acuracy.  Less is more (1 is horribly awful, .0001 is close to perfect)
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_mechasaurus_mk4.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_mechasaurus_mk4.mdl" -- Worldmodel path
...
SWEP.Primary.ImpactEffect = "exp_mechasaurus_mk4"
...
SWEP.Base				= "tfa_gun_base"
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_mechasaurus_mk4")
...
function SWEP:SecondaryAttack()
	if self:Ammo2() < 75 then return end
...
function SWEP:Think2(...)
	if self:GetStatus() == TFA.Enum.STATUS_SILENCER_TOGGLE and self:GetStatusEnd() <= CurTime() then
		...
		if self:GetSilenced() then
			self.Primary_TFA.Damage = self.Primary.Damage_Regular
			...
			self.Primary_TFA.Damage = self.Primary.Damage_BMode
			self.Primary_TFA.RPM = 700
			self.Primary_TFA.AmmoConsumption = 0
			self.Primary_TFA.NumShots = 1
			self.MuzzleFlashEffectSilenced = "muz_mechasaurus_b"
		    self:ClearStatCache()
        end
    end
...
function SWEP:Reload( ... )
	if self:GetSilenced() and not self.Owner:KeyDown(IN_USE) then
		if self:GetStatus() == TFA.GetStatus("idle") and self:Ammo2() < 0 then
...
SWEP.Secondary.Ammo			= "mecha_mk4" -- Self explanitory, ammo type.

function SWEP:PostPrimaryAttack()
self.NextGen = self.NextGen +1
if SERVER then
if self.NextGen >= 1 and not self:GetSilenced() and self:Ammo2() < 75 then
self.Owner:GiveAmmo( 3,self.Secondary.Ammo, true )           
self.NextGen  = 0
end
...
if self:Ammo2() == 75 and self.NextGen == 0 then
self:EmitSound("MechasaurusMK.Ready")
end
if self:Ammo2() == 0 and self:GetSilenced() then
		self:ChooseSilenceAnim( not self:GetSilenced() )
		self:SetStatus(TFA.Enum.STATUS_SILENCER_TOGGLE)
		self:SetStatusEnd(CurTime() + 1)
end
if self:Ammo2() == 71 and self:GetSilenced() then
self.Primary_TFA.Damage = self.Primary.Damage_Secondary
self.Primary_TFA.DamageType = bit.bor(DMG_BLAST,DMG_AIRBOAT,DMG_ALWAYSGIB)
self.Primary_TFA.BlastRadius	= 125
end
if self:Ammo2() == 70 and self:GetSilenced() then
self.Primary_TFA.Damage = self.Primary.Damage_BMode
self.Primary_TFA.DamageType = DMG_BULLET
end
if self:Ammo2() == 61 and self:GetSilenced() then
self.Primary_TFA.Damage = self.Primary.Damage_Secondary
self.Primary_TFA.DamageType = bit.bor(DMG_BLAST,DMG_AIRBOAT,DMG_ALWAYSGIB)
self.Primary_TFA.BlastRadius	= 125
end
if self:Ammo2() == 60 and self:GetSilenced() then
self.Primary_TFA.Damage = self.Primary.Damage_BMode
self.Primary_TFA.DamageType = DMG_BULLET
end
if self:Ammo2() == 51 and self:GetSilenced() then
self.Primary_TFA.Damage = self.Primary.Damage_Secondary
self.Primary_TFA.DamageType = bit.bor(DMG_BLAST,DMG_AIRBOAT,DMG_ALWAYSGIB)
self.Primary_TFA.BlastRadius	= 125
end
if self:Ammo2() == 50 and self:GetSilenced() then
self.Primary_TFA.Damage = self.Primary.Damage_BMode
self.Primary_TFA.DamageType = DMG_BULLET
end
if self:Ammo2() == 41 and self:GetSilenced() then
self.Primary_TFA.Damage = self.Primary.Damage_Secondary
self.Primary_TFA.DamageType = bit.bor(DMG_BLAST,DMG_AIRBOAT,DMG_ALWAYSGIB)
self.Primary_TFA.BlastRadius	= 125
end
if self:Ammo2() == 40 and self:GetSilenced() then
self.Primary_TFA.Damage = self.Primary.Damage_BMode
self.Primary_TFA.DamageType = DMG_BULLET
end
if self:Ammo2() == 31 and self:GetSilenced() then
self.Primary_TFA.Damage = self.Primary.Damage_Secondary
self.Primary_TFA.DamageType = bit.bor(DMG_BLAST,DMG_AIRBOAT,DMG_ALWAYSGIB)
self.Primary_TFA.BlastRadius	= 125
end
if self:Ammo2() == 30 and self:GetSilenced() then
self.Primary_TFA.Damage = self.Primary.Damage_BMode
self.Primary_TFA.DamageType = DMG_BULLET
end
if self:Ammo2() == 21 and self:GetSilenced() then
self.Primary_TFA.Damage = self.Primary.Damage_Secondary
self.Primary_TFA.DamageType = bit.bor(DMG_BLAST,DMG_AIRBOAT,DMG_ALWAYSGIB)
self.Primary_TFA.BlastRadius	= 125
end
if self:Ammo2() == 20 and self:GetSilenced() then
self.Primary_TFA.Damage = self.Primary.Damage_BMode
self.Primary_TFA.DamageType = DMG_BULLET
end
if self:Ammo2() == 11 and self:GetSilenced() then
self.Primary_TFA.Damage = self.Primary.Damage_Secondary
self.Primary_TFA.DamageType = bit.bor(DMG_BLAST,DMG_AIRBOAT,DMG_ALWAYSGIB)
self.Primary_TFA.BlastRadius	= 125
end
if self:Ammo2() == 10 and self:GetSilenced() then
self.Primary_TFA.Damage = self.Primary.Damage_BMode
self.Primary_TFA.DamageType = DMG_BULLET
end
if self:Ammo2() == 1 and self:GetSilenced() then
self.Primary_TFA.Damage = self.Primary.Damage_Secondary
self.Primary_TFA.DamageType = bit.bor(DMG_BLAST,DMG_AIRBOAT,DMG_ALWAYSGIB)
self.Primary_TFA.BlastRadius	= 125
end
