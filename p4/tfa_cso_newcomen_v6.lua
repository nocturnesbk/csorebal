...
function SWEP:SecondaryAttack( ... )
        if self:Clip1() > 0 and not self:GetSprinting() and TFA.Enum.ReadyStatus[ self:GetStatus() ] then
			...
			self.Primary_TFA.Sound 	= Sound("Newcomen.Fire2")
        ...
        elseif self:GetOwner():KeyDown(IN_ATTACK2) then
            ...
            self.Primary_TFA.RPM = 900
            self.Primary_TFA.KickUp    = 0.08
            self.Primary_TFA.KickDown    = 0.02
            self.Primary_TFA.KickHorizontal    = 0.08
            self.Primary_TFA.Spread        = .008
			self.Primary_TFA.Sound 	= Sound("Newcomen.Fire")
