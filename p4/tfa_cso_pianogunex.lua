...
SWEP.Author				= "Kamikaze, Bread" --Author Tooltip
...
SWEP.PrintName				= "Psychic Sizer"		-- Weapon name (Shown on HUD)
...
--Firing related
SWEP.Primary.Sound 			= Sound("Pianogunex.Fire1")				-- This is the sound of the weapon, when you shoot.
...
SWEP.NZPaPName				= "Synthetic Musical"
...
SWEP.ProjectileEntity = "pianogunex" --Entity to shoot
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_pianogunex.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_pianogunex_right.mdl" -- Worldmodel path
...
SWEP.WElements = {
	["other_pianogun"] = { type = "Model", model = "models/weapons/tfa_cso/w_pianogunex_left.mdl", bone = "ValveBiped.Bip01_L_Hand", rel = "", pos = Vector(2.25, -2, 4.55), angle = Angle(0, 80, 0), size = Vector(1.4, 1.4, 1.4), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
SWEP.MuzzleFlashEffect = "cso_muz_pianogunex_a" --Change to a string of your muzzle flash effect.  Copy/paste one of the existing from the base.
...
SWEP.VElements = {
	["reload"] = { type = "Model", model = "models/hunter/plates/plate1x1.mdl", bone = "Point_crosshair", rel = "", pos = Vector(0, 0, 0), angle = Angle(0, 0, 0), size = Vector(0.25, 0.25, 0), color = Color(255, 255, 255, 255), surpresslightning = false, material = "sprites/ef_pianogunex_muzzle01", skin = 0, bodygroup = {}, active = false },
}
...
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_pianogunex")
...
function SWEP:SecondaryAttack(...)
   ...
   self:EmitSound("Pianogunex.Fire2")


   if self.combo < self.comboright + self.comboleft then
       self.combo = self.combo + 1
       if SERVER then
        local dmginfo = CSO:DMGInfo(self.Owner, self, 1750, Vector(0,0,0), DMG_BLAST, self:GetPos())
        CSO:DoRadiusAttack(self, dmginfo, 350, true, 2048, 1.5, false, nil, true)
       end
       if IsFirstTimePredicted() then
         local effectdata = EffectData()
         effectdata:SetOrigin(self.Owner:GetPos())
         util.Effect("exp_psychic", effectdata) -- easy effect
       end
   elseif self.combo >= self.comboright + self.comboleft then
       ...
       if SERVER then
        local dmginfo = CSO:DMGInfo(self.Owner, self, 1750, Vector(0,0,0), DMG_BLAST, self:GetPos())
        CSO:DoRadiusAttack(self, dmginfo, 350, true, 2048, 2, false, nil, true)
       end
       if IsFirstTimePredicted() then
         local effectdata = EffectData()
         effectdata:SetOrigin(self.Owner:GetPos())
...
hook.Add( "PlayerShouldTakeDamage", "antiselfdamage_pianogunex", function( ply, attacker ) -- I want to remove this hook but projectiles will self-damage
   if ply == attacker and (ply:GetActiveWeapon():GetClass() == "tfa_cso_pianogunex") then -- grumble grumble
      return false
   end
end )