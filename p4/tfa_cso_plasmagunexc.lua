...
SWEP.Purpose				= " X (Suit Zoom Button) = Secondary Mode " --Purpose Tooltip
...
SWEP.Primary.DamageType = bit.bor(DMG_AIRBOAT,DMG_BLAST,DMG_ALWAYSGIB) --See DMG enum.  This might be DMG_SHOCK, DMG_BURN, DMG_BULLET, etc.
...
SWEP.MuzzleFlashEffectSilenced = "none"
...
function SWEP:Initialize()
	...
	self.StatCache_Blacklist["Primary.DamageTypeHandled"] = true
...
function SWEP:Think2(...)
	if self:GetStatus() == TFA.Enum.STATUS_SILENCER_TOGGLE and self:GetStatusEnd() <= CurTime() then
		...
		self.Primary_TFA.Sound = Sound("TiamatMK3.Fire")
		self.Primary_TFA.AmmoConsumption = 1
		self.Primary_TFA.Projectile = "plasmaexc"
		self.Primary_TFA.ProjectileVelocity = 6500
		self:ClearStatCache()
		else
		self.Primary_TFA.Damage = 1000
		self.Primary_TFA.DaamgeType = bit.bor(DMG_AIRBOAT,DMG_BLAST,DMG_ALWAYSGIB)
        self.Primary.DamageTypeHandled = false
		self.Primary_TFA.Sound = Sound("TiamatMK3.Fire2")
		self.Primary_TFA.AmmoConsumption = 1
		self.TracerName = "cso_tra_scsr"
