...
SWEP.PrintName				= "Poison Launcher"		-- Weapon name (Shown on HUD)
...
--[[VIEWMODEL]]--
SWEP.ViewModel			= "models/weapons/tfa_cso/c_poisongun.mdl" --Viewmodel path
...
--}
--[[WORLDMODEL]]--
SWEP.WorldModel			= "models/weapons/tfa_cso/w_poisongun.mdl" -- Weapon world model path
...
CreateClientConVar( "cso_poisongun_fx", 2, false, false )
...
function SWEP:PrimaryAttack(...)
	...
	util.Effect("cso_poisongun",flamefx,true,true)
end
...
function SWEP:Think2( ... )
	...
	if shooting ~= self.Shooting_Old then
		if shooting then
			self:EmitSound("Poisongun.Fire")
			self.NextIdleSound = CurTime() + 0.2
		else
			self:EmitSound("Poisongun.Fire_End")
	...
	if shooting then
		if self.NextIdleSound and CurTime() > self.NextIdleSound then
			self:EmitSound("Poisongun.Fire")
			self.NextIdleSound = CurTime() + SoundDuration( "Poisongun.Fire" ) - 0.1
		end
	end
...
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_poisongun")
