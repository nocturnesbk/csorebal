...
function SWEP:SecondaryAttack()
	local skillt = self:GetNWInt("skillt", 0)
	local skill = self:GetNWInt("skill", 1)
	local skilltime = self:GetNWFloat("skilltime", 0)
	local skilltime1 = self:GetNWFloat("skilltime1", 0)
	...
	-- print(skillt, skill, skilltime, skilltime1)
    if self:Clip1() > 1 then
        ...
        if skilltime < CurTime() then
            ...
            -- self.skilltime = CurTime() + 0.5
        end

        if skilltime1 < CurTime() then
			self:SetNWInt("skillt", skillt + 1)
            -- self.skillt = self.skillt + 1
            if skillt > 5 then
				self:SetNWInt("skillt", 0)
                -- self.skillt = 0
            end
			self:SetNWFloat("skilltime1", CurTime() + 0.2)
			-- self.skilltime1 = CurTime() + 0.2
        end

        timer.Create( "rainbowkatadeploy"..self:EntIndex(), 1, 1, function()
				...
				for k, target in pairs( ents.FindInSphere( self.Owner:GetPos() + self.Owner:OBBCenter(), radius ) ) do
					if IsValid( target ) then
						...
						if self:IsValidTarget(target) then
							...
							if target != self.Owner and ( ( !target.was_pushed || target.was_pushed.t != CurTime() ) ) then
