...
--Firing related
SWEP.Primary.Sound 			= Sound("StarTaylor.Fire")				-- This is the sound of the weapon, when you shoot.
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_star_taylor.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_star_taylor.mdl" -- Worldmodel path
...
SWEP.TracerName 		= "cso_tra_star_taylor" 	--Change to a string of your tracer name.  Can be custom.
...
function SWEP:Holster( ... )
	self:StopSound("StarTaylor.Idle")
	return BaseClass.Holster(self,...)
end
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_star_taylor")
...
function SWEP:Think2( ... )
    ...
    if shooting ~= self.Shooting_Old then
        if shooting then
            self:EmitSound("StarTaylor.Fire")
            self.NextIdleSound = CurTime() + 0.2
        else
            self:EmitSound("StarTaylor.Fire_End")
            self:StopSound("StarTaylor.Idle")
    ...
    if shooting then
        if self.NextIdleSound and CurTime() > self.NextIdleSound then
            self:EmitSound("StarTaylor.Fire")
            self.NextIdleSound = CurTime() + SoundDuration( "StarTaylor.Fire" ) - 0.1
        end
    end
