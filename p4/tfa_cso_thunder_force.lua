...
SWEP.PrintName				= "Thunder Force"		-- Weapon name (Shown on HUD)
...
--Firing related
SWEP.Primary.Sound 			= Sound("ThunderForce.Fire")				-- This is the sound of the weapon, when you shoot.
SWEP.Primary.Damage_Regular		= 120					-- Damage, in standard damage points.
SWEP.Primary.Damage		= SWEP.Primary.Damage_Regular
SWEP.DamageType = bit.bor(DMG_BLAST,DMG_BULLET,DMG_ALWAYSGIB) --See DMG enum.  This might be DMG_SHOCK, DMG_BURN, DMG_BULLET, etc.
SWEP.Primary.Damage_Secondary		= 960				-- Damage, in standard damage points.
SWEP.Primary.NumShots	= 1 --The number of shots the weapon fires.  SWEP.Shotgun is NOT required for this to be >1.
SWEP.Primary.Automatic			= true					-- Automatic/Semi Auto
SWEP.Primary.HullSize = 0
SWEP.Primary.BlastRadius	= 15               -- Radius for DMG_BLAST
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_thunder_force.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_thunder_force.mdl" -- Worldmodel path
...
SWEP.MuzzleFlashEffect = "cso_muz_thunderforce" --Change to a string of your muzzle flash effect.  Copy/paste one of the existing from the base.

SWEP.ImpactEffect = "tfa_metal_impact"--Impact Effect
SWEP.ImpactDecal = nil--Impact Decal
SWEP.Primary.ImpactEffect = "exp_thunderforce"

--Tracer Stuff

SWEP.Tracer				= 0		--Bullet tracer.  TracerName overrides this.
SWEP.TracerName 		= "cso_tra_thunderforce" 	--Change to a string of your tracer name.  Can be custom.
...
SWEP.Base				= "tfa_gun_base"
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_thunder_force")
...
function SWEP:Initialize()
	...
	self.StatCache_Blacklist["DrawCrosshairIS"] = true
...
function SWEP:Think2(...)
	if self:GetIronSightsProgress() <= 0.5 and self:GetCSO_SwitchState() then
		...
		self.Primary_TFA.Sound = Sound("ThunderForce.Fire")
		self.TracerName = "cso_tra_thunderforce"
		self.Primary_TFA.DamageType = bit.bor(DMG_BLAST,DMG_BULLET,DMG_ALWAYSGIB)
		self.Primary_TFA.BlastRadius = 15
		self.Primary_TFA.Hullsize = 0
		...
		self.MuzzleFlashEffect = "cso_muz_thunderforce"
		self.Primary_TFA.ImpactEffect = "exp_thunderforce"
		self:SetCSO_SwitchState(false)
		self:ClearStatCache()
	elseif self:GetIronSightsProgress() > 0.5 and not self:GetCSO_SwitchState() then
		self.TracerName = "tra_thunderforce"
		...
		self.Primary_TFA.Sound = Sound("ThunderForce.Fire2")
		self.Primary_TFA.BlastRadius = 100
		self.Primary_TFA.Hullsize = 2
		self.Primary_TFA.RPM = 80
		self.Primary_TFA.Spread = 0.005
		self.Primary_TFA.AmmoConsumption = 5
		self.Primary_TFA.NumShots = 1
		self.DrawCrosshairIS = true
		self.MuzzleFlashEffect = "cso_muz_thunderforce2"
		self.Primary_TFA.ImpactEffect = "exp_thunderforce2"
