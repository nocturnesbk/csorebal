...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_voidpistol.mdl" -- same case UHGI;DFSD/
...
SWEP.MuzzleFlashEffect = "cso_muz_void" --Change to a string of your muzzle flash effect.  Copy/paste one of the existing from the base.

--Tracer Stuff

SWEP.Tracer				= 0		--Bullet tracer.  TracerName overrides this.
SWEP.TracerName 		= "cso_tra_void" 	--Change to a string of your tracer name.  Can be custom.
...
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_voidpistol")
...
SWEP.Secondary.Ammo = "blackhole"
SWEP.Secondary.Ent = "voidavenger_blackhole"
SWEP.Secondary.Automatic = true
SWEP.Secondary.Model = "models/items/ar2_grenade.mdl" --Projectile Model
SWEP.Secondary.Damage = 225 --Grenade Damage
...
function SWEP:PostPrimaryAttack()
	...
	for k,v in pairs(ents.FindInSphere(self:GetOwner():GetPos(), 416)) do
...
function SWEP:Think2(...)
	if self:GetStatus() == TFA.Enum.STATUS_SILENCER_TOGGLE and self:GetStatusEnd() <= CurTime() then
		...
		if self:GetSilenced() then
			...
			self.MuzzleFlashEffect = "cso_muz_void"
			self.TracerName = "cso_tra_void"
		else
			self.Primary_TFA.AmmoConsumption = 1
			self.Primary_TFA.NumShots = 1
			self.Primary_TFA.HullSize = 10
			self.MuzzleFlashEffectSilenced = "cso_muz_void"
			self.TracerName = "cso_tra_void"
		end
	end
